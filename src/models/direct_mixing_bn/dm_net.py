"""
Direct Mixing ResNet (DMNet) implementations.

Supports N input streams with scalar-based direct mixing for integration.
...
# Import the direct mixing components
from .conv import DMConv2d, DMBatchNorm2d
from .blocks import DMBasicBlock, DMBottleneck
from .container import DMSequential, DMReLU
from .pooling import DMMaxPool2d, DMAdaptiveAvgPool2d
# Note: DMNet doesn't use fusion - it uses the integrated stream directly


class DMNet(BaseModel):
    """
    Direct Mixing ResNet (DMNet) implementation.

    Supports N input streams with scalar-based direct mixing for integration:
    - N independent stream pathways (e.g., RGB, Depth, Orthogonal, etc.)
    - Integrated: Scalar-weighted fusion pathway (combines stream outputs using learnable scalars α, γ)

    Uses unified DMConv2d neurons where integration happens INSIDE the neuron via scalar multiplication.
    ...
    def __init__(
        self,
        block: type[Union[DMBasicBlock, DMBottleneck]],
        layers: list[int],
        num_classes: int,
        ...
        use_amp: bool = False,
        # DMNet-specific parameters come AFTER all ResNet parameters
        ...
        **kwargs
        ...
        # Store DMNet-specific parameters BEFORE calling super().__init__
        ...
        # Set DMNet default norm layer if not specified
    ...
    def _build_network(
        ...
        """Build the Direct Mixing ResNet network architecture."""
        ...
        # Network architecture - exactly like ResNet but with N-stream DM components
        ...
        self.maxpool = DMMaxPool2d(kernel_size=3, stride=2, padding=1)
    ...
    def _initialize_weights(self, zero_init_residual: bool):
        ...
        # Weight initialization for DMConv2d (already handled in DMConv2d.reset_parameters())
        # Scalar mixing weights (α, γ) are initialized in DMConv2d.__init__
        for m in self.modules():
            if isinstance(m, DMConv2d):
                # DMConv2d.reset_parameters() already initializes all weight matrices
                # Scalar mixing weights (α_i=1.0, γ=0.2) are set in __init__
                pass
            elif isinstance(m, DMBatchNorm2d):
                # DMBatchNorm2d uses standard nn.BatchNorm2d internally,
    ...
    def _make_layer(
        ...
        Fully compliant with ResNet implementation but using DM blocks with N-stream channel tracking.
        """
        ...
        return DMSequential(*layers)

    def forward(self, stream_inputs: list[Tensor]) -> Tensor:
        """
        Forward pass through the Direct Mixing ResNet.
        ...
        """
        # Initial convolution (creates integrated stream from all input streams using scalar mixing)
        ...
        # ResNet layers (all N streams, integration happens inside DMConv2d neurons using scalar mixing!)
    ...
    def fusion_strategy(self) -> str:
        ...
        For DMNet, integration happens inside DMConv2d neurons using scalar mixing (not separate fusion layer).

        Returns:
            A string representing the fusion type.
        """
        return "direct_mixing"
...
def dm_net18(num_classes: int = 1000, stream_input_channels: list[int] = None, **kwargs) -> DMNet:
    ...
    Args:
        ...
        **kwargs: Additional arguments passed to DMNet constructor

    Returns:
        DMNet model instance

    Examples:
        # 2-stream (backward compatible - default)
        model = dm_net18(num_classes=15)
        # 3-stream (RGB + Depth + Orthogonal)
        model = dm_net18(num_classes=15, stream_input_channels=[3, 1, 1])
    """
...
def dm_net34(num_classes: int = 1000, stream_input_channels: list[int] = None, **kwargs) -> DMNet:
    """
    Create a Direct Mixing ResNet-34 model.
...
def dm_net50(num_classes: int = 1000, stream_input_channels: list[int] = None, **kwargs) -> DMNet:
    """
    Create a Direct Mixing ResNet-50 model.
...
def dm_net101(num_classes: int = 1000, stream_input_channels: list[int] = None, **kwargs) -> DMNet:
    """
    Create a Direct Mixing ResNet-101 model.
